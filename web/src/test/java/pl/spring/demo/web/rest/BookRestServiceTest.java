...
public class BookRestServiceTest {
	...
	public void testShouldCallBookService() throws Exception {
		...
		ResultActions response = this.mockMvc.perform(get("/books-by-title?titlePrefix=" + bookTitle)
				.accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON)); // symuluje
																								// wywolanie
		// then
		...
		.andExpect(jsonPath("[0].id").value(bookTo1.getId().intValue())) // porownuje
																			// stworzone
				.andExpect(jsonPath("[0].title").value(bookTo1.getTitle())) // obiekty
																			// do
																			// tych
																			// znalezion
				.andExpect(jsonPath("[0].authors").value(bookTo1.getAuthors())) // ych
																				// za
																				// pomoca
																				// tytulu
																				// i
																				...
																				// w
																				// jsonowej
				.andExpect(jsonPath("[1].id").value(bookTo2.getId().intValue())) // tablicy
	...
	public void testShouldDeleteBook() throws Exception {
		final String createJson = "{\"id\":\"4\",\"title\":\"Title1\",\"authors\":\"Author1\"}";
		// when
		this.mockMvc.perform(post("/book").accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON)
				.content(createJson.getBytes()));
		// when
		ResultActions response = this.mockMvc.perform(delete("/book-delete").accept(MediaType.APPLICATION_JSON)
				.contentType(MediaType.APPLICATION_JSON).content(createJson.getBytes()));
		// then
		response.andExpect(status().isOk());
	}
}
